...
class McpServer:
    ...
    def add_common_tools(self):
        ...
        # -------------------------- 新增：注册calculate工具 --------------------------
        from src.mcp.tools.calculate import get_calculate_manager

        calculate_manager = get_calculate_manager()
        calculate_manager.init_tools(self.add_tool, PropertyList, Property, PropertyType)

        # -------------------------- 新增：注册VLN工具 --------------------------
        from src.mcp.tools.navigation import get_navigation_manager

        navigation_manager = get_navigation_manager()
        navigation_manager.init_tools(self.add_tool, PropertyList, Property, PropertyType)
        
        # -------------------------- 新增：注册knowledgebase工具 --------------------------
        from src.mcp.tools.knowledgebase import get_knowledgebase_manager

        knowledgebase_manager = get_knowledgebase_manager()
        knowledgebase_manager.init_tools(self.add_tool, PropertyList, Property, PropertyType)
        
        # -------------------------- 新增：注册action工具 -------------------------------
        from src.mcp.tools.action import get_action_manager

        action_manager = get_action_manager()
        action_manager.init_tools(self.add_tool, PropertyList, Property, PropertyType)
